...
| :--- | :--- | :--- |
| [`index.html`](./index.html) | トップページ | ヒーロー（キャッチコピー）、コンセプト（Virtual to Real）、提供サービス一覧（Our Services）、参戦カテゴリー一覧、NEWS（新着情報・入庫・レポート）、ドライバー・体制紹介、実績、問い合わせ・アクセス |
...
- **サイト全体との連動**:
  - 全ページの共通ナビゲーション（ヘッダー・スマホメニュー）に「ロードマップ」リンクを設置し、各カテゴリーやサービスから相互アクセス可能。

### 4.7 モバイル用固定アクションバー（`<aside>`）
スマートフォン・タブレット等のモバイル端末閲覧時、画面最下部に常時固定されるコンバージョン用フローティングバー（`lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md`）。
- **目的**: 縦長ページの閲覧中、フッターまでスクロールしなくてもワンタップで電話発信やお問い合わせ、予約・アクセスへ誘導し、離脱を最小化する。
- **ボタン構成（3カラム）**:
  1. **電話相談**: `tel:08011724441`（通話発信）
  2. **主CTA（お問い合わせ / 予約・相談）**: `bg-rose-600 text-white font-bold`（各ページに応じた文脈の予約・問い合わせへスクロールまたは遷移）
  3. **アクセス / 店舗案内**: `#contact`（フッターの住所・マップへスクロール）
- **安全領域・被り防止**:
  - 各ページの `<body>` に `pb-20 lg:pb-0` を付与し、フッター下部のコンテンツがバーに隠れないようレイアウト余白を確保。
  - iOS端末のホームバーを考慮し、`pb-[max(0.5rem,env(safe-area-inset-bottom))]` を設定。
