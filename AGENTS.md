...
- **User Communication:**
  ...
  - お世辞、過剰な賞賛、おだて（へつらい）の表現は一切使用しないこと。
  - 感情的な装飾を排し、事実に基づき客観的かつ簡潔に応答すること。
...
- **Git & System Commands:**
  ...
  - `node -e` 等のワンライナー実行は承認ダイアログの原因となるため使用を禁止する。
  - バイナリやファイルからの文字列抽出・調査には `rg --text -C <N> "検索ワード" "path"`、または固定スクリプトファイルを使用すること。